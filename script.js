...
const buildings = [
  { src: '/buildings/palace1.png', width: 1000, height: 1000 },
  { src: 'building2.png', width: 1000, height: 1000 },
];
...
canvas.addEventListener('click', (event) => {
  if (selectedBuilding) {
    ...
    image.src = selectedBuilding.src;
    ctx.drawImage(image, x, y, selectedBuilding.width, selectedBuilding.height);
    console.log('placed');
  }
});
...
// button to add image to canvas
const addImage = document.getElementById('add-image');
addImage.addEventListener('click', () => {
  const x = Math.random() * 1000;
  const y = Math.random() * 1000;
  addImageToCanvas('/buildings/palace1.png', x, y);
});
